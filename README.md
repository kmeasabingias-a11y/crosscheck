...
[![CI](https://github.com/kmeasabingias-a11y/crosscheck/actions/workflows/ci.yml/badge.svg)](https://github.com/kmeasabingias-a11y/crosscheck/actions/workflows/ci.yml)
...
### Measured three ways, and the gaps are published

- **Injected benchmark** — 139 pairs, generated by GPT-4.1 and judged by Claude, deliberately
  different model families so the score is not self-recognition. F1 **.745**. This is the ceiling.
- **Hand-authored benchmark** — 28 pairs written in realistic phrasing. F1 **.578**. Injected
  contradictions are lexically cleaner than real drift, and this is what that costs.
- **A real published standard** — NIST SP 800-63B Rev 3 vs Rev 4, unlabelled and never seen by the
  system. 4 genuine findings of 15: **26.7% precision**. Two of them are quoted below.

Also here: a [confidence calibration plot](#calibration) with ECE per bin, [ablations](#ablations)
that overturned one of this project's own design assumptions, and a
[decision log](DECISIONS.md) of 61 entries — including the ones that turned out to be wrong.
...
The two amber stages cost money; the blue one is what keeps that cost down. Everything else runs
locally.
