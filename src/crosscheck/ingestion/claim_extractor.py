...
def _finalize_claim(raw: ExtractedClaim, chunk: Chunk) -> Claim | None:
    ...
    The evidence quote must be a verbatim, non-empty substring of the chunk; its
    position gives the offset, from which the claim id is derived.
    """
    quote = raw.evidence_quote
    start = chunk.text.find(quote) if quote.strip() else -1
    if start < 0:
        logger.warning(
            "extractor: evidence quote not found verbatim in chunk {} — dropping claim: {!r}",
            chunk.chunk_id,
            quote,
        )
        return None
    ...
    return Claim(
        claim_id=claim_id(chunk.doc_id, chunk.chunk_id, offset),
        doc_id=chunk.doc_id,
